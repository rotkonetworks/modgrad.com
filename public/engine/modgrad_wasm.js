/* @ts-self-types="./modgrad_wasm.d.ts" */

/**
 * @param {number} _chosen
 * @param {number} _signal
 * @returns {number}
 */
export function apply_plasticity(_chosen, _signal) {
    const ret = wasm.apply_plasticity(_chosen, _signal);
    if (ret[2]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('modgrad_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
